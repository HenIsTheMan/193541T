...
void App::RenderSceneToCreatedFB(const Cam& cam, const Framebuffer* const& FBO, const Framebuffer* const& enFBO, const uint* const& depthTexs, const short& index) const{ //Create framebuffer (combination of render buffers stored in GPU mem) to get an extra target to render to
    glBindFramebuffer(GL_FRAMEBUFFER, FBO->GetRefID()); {
        ...
        scene->RenderToCreatedFB(cam, enFBO ? &(enFBO->GetTexColourBuffer().GetRefID()) : 0, depthTexs);
    } glBindFramebuffer(GL_FRAMEBUFFER, 0);
}

void App::RenderSceneToDefaultFB(const Framebuffer* const& readFBO, const Framebuffer* const& writeFBO, const Framebuffer* const& screenFBO, const glm::vec3&& translate, const glm::vec3&& scale) const{
    ...
    scene->RenderToDefaultFB(screenFBO->GetRefID(), translate, scale); //Some post-processing filters like edge detection will produce jagged edges as screenTex is non-multisampled so do post-processing later or use anti-aliasing alg to correct
}
...
void App::Init(){
    ...
    enFBO = new Framebuffer(GL_TEXTURE_CUBE_MAP, 1, 2048, 2048, GL_LINEAR, GL_LINEAR, GL_CLAMP_TO_EDGE);
...
void App::Render(const Cam& cam) const{
    ...
    glViewport(0, 0, 1024, 1024); { //Render scene in depth/... map resolution so depth/... map will be neither incomplete nor too small
        //glCullFace(GL_FRONT); { //Solves peter panning (shadow mapping artefact, shadow bias applied leads to visible offset of obj shadows, adjust shadow bias to avoid) for solid objs but causes shadows of plane objs to disappear //Take depth of back faces so shadows will form inside objs??
            RenderSceneToCreatedFB(Cam(glm::vec3(-2.f, 4.f, -1.f), glm::vec3(0.f), 0), dDepthMapFBO, enFBO, 0);
    ...
    uint depthTexs[]{dDepthMapFBO->GetTexColourBuffer().GetRefID(), sDepthMapFBO->GetTexColourBuffer().GetRefID()};
    ...
    Cam enCam(glm::vec3(.1f, 0.f, .1f), glm::vec3(0.f), 2, 1.f);
    ...
    RenderSceneToCreatedFB(Cam(cam.GetPos(), cam.GetPos() + cam.GetPos() - cam.GetTarget()), backFBO, enFBO, depthTexs);
    glClear(GL_COLOR_BUFFER_BIT);
    RenderSceneToDefaultFB(frontFBO, intermediateFBO, intermediateFBO);
    //RenderSceneToDefaultFB(0, 0, depthTexS);
    RenderSceneToDefaultFB(backFBO, intermediateFBO, intermediateFBO, glm::vec3(-.5f, .5f, 0.f), glm::vec3(.5f));
}
