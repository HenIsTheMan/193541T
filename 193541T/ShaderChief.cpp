...
void ShaderChief::SetUni1f(cstr uniName, float value) const{
...
void ShaderChief::SetUniMtx4fv(cstr uniName, float* address) const{
	int uniLocation = glGetUniformLocation(shaderProgID, uniName); //Query location of uniform
	if(uniLocation != -1){
...
void ShaderChief::SetUni3f(cstr uniName, float value1, float value2, float value3) const{
	int uniLocation = glGetUniformLocation(shaderProgID, uniName); //Query location of uniform
	if(uniLocation != -1){
...
void ShaderChief::SetUni4f(cstr uniName, float values[4]) const{
	int uniLocation = glGetUniformLocation(shaderProgID, uniName); //Query location of uniform
	if(uniLocation != -1){
...
void ShaderChief::SetUni1i(cstr uniName, int value) const{
	int uniLocation = glGetUniformLocation(shaderProgID, uniName); //Query location of uniform
	if(uniLocation != -1){
